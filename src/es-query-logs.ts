 ...
 */
const TRACE_FIELDS = ['trace_id', 'trace.id', 'traceId', 'traceID'] as const
 ...
 */
const DEFAULT_TEXT_FIELDS = ['message', 'log', 'exception', 'stack_trace', 'error', 'msg', 'details', 'reason'] as const
...
/** The subset of an ES `_search` response this tool reads. */
...
function totalOf(data: EsSearchResponse, hitCount: number): number {
 ...
 */
function resolveFields(fields: readonly string[] | undefined): readonly string[] {
  if (fields === undefined || fields.length === 0) return DEFAULT_TEXT_FIELDS
...
function describeCriteria(traceId: string, searchText: string): string {
  const parts: string[] = []
...
function formatHits(data: EsSearchResponse, uri: string, descriptor: string): string {
...
function readConfig(ctx: Context): ElasticsearchSettings {
  const resolved = ctx.settings.get(NAMESPACE) as Partial<ElasticsearchSettings> | undefined
  return {
...
async function search(uri: string, index: string, body: string, config: ElasticsearchSettings, signal: AbortSignal): Promise<EsSearchResponse> {
...
function clampSize(size: number | undefined): number {
  if (size === undefined) return ES_QUERY_LOGS_DEFAULT_SIZE
  if (!Number.isInteger(size) || size < 1) return ES_QUERY_LOGS_DEFAULT_SIZE
 ...
 */
export function applyEsQueryLogsTool(ctx: Context): void {
